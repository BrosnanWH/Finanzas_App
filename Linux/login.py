...
def verificar_login():
    ...
    try:
        ...
        conexion = mysql.connector.connect(
            host="localhost",  # Cambia si tu servidor MySQL no es local
            user="J5h4",       # Cambia por tu usuario de MySQL
            password="",       # Cambia por tu contraseña de MySQL
            database="Northwind"  # Cambia si tu base de datos tiene otro nombre
        )
        cursor = conexion.cursor()

        # Consulta para verificar las credenciales (ajusta según tu tabla Usuarios)
        cursor.execute("SELECT * FROM Usuario_login WHERE usuario = %s AND contrasena = %s", (usuario, contrasena))
        if cursor.fetchone():
